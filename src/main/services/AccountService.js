...
module.exports = app => {
  ...
  const { deleteCharacter, updateHidenCharacterInDB, updateCharacterCommentInDB, 
    checkIfExixtsNameOrEmailOrBothAndReturnAccount, validateJsonTokenRepository } = app.src.main.repository.AccountRepository;
  ...
  const recoveryAccountGenericService = async (data) => {
    ...
    console.log('consolando data no service: ', data);
    ...
    if (data.trigger === 'emailCheck') {
      ...
      if (returnal.status === 200) {
        ...
        const link = `http://${process.env.BASE_URL_IP}:3000/account-changepassword/${hashGenerator(16)}-${acc.id}`;
        try {
