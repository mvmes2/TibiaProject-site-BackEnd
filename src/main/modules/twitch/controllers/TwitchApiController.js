...
module.exports = app => {
	...
	const twitch = async (req, res) => {
		...
		livesStremandoTibiaProject.map(async (item) => {
			...
			if (oficialStreamersList?.length > 0 && oficialStreamersList.some((streamerSome) => streamerSome.twitch_user_id == item.user_id) && !checkIfAlreadyExistLiveCheck.find((find) => find.live_id == item.id)) {
