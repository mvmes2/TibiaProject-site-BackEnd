...
const storageContract = multer.diskStorage({
    destination: (req, file, cb) => {
        const destinationPath = path.join(__dirname, '..', 'resources', 'contract-images');
    ...
    try {
        if (req.files && req.files.length > 0) {
            for (let i = 0; i < req.files.length; i++) {
                ...
                const outputPath = path.join(__dirname, '..', 'resources', 'contract-images', 'compressed', outputFileName);

                // Verificar e criar o diretório 'compressed', se necessário
                const compressedDir = path.join(__dirname, '..', 'resources', 'contract-images', 'compressed');
