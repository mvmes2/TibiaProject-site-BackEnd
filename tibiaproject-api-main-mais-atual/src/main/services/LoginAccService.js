...
module.exports = app => {
	...
	const LoginAccService = async (data) => {
		...
		const newLoginToken = generateToken(60, accwithoutPassword);
		...
		return { status: 200, message: { id: acc.id, login_hash: hash, name: acc.name, login_token: newLoginToken, email: acc.email, country: acc.country, coins: acc.coins, coins_tranferable: acc.coins_transferable } };
	}
	return {
		LoginAccService,
	}
