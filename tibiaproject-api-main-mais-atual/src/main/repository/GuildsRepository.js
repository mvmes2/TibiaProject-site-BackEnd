...
const getGuildList = async () => {
  ...
  try {
    ...
    .leftJoin('tibiaprojectslave.guild_logos', 'guilds.id', '=', 'guild_logos.guild_id')
...
const createNewGuild = async (data, validatedAccountID) => {
  ...
  try {
    ...
    const checkIfHaveCoinsAndTRansferableCoins = await accounts.query().select('coins', 'coins_transferable').where({ id: data.account_id }).first();
    ...
    if (Number(checkIfHaveCoinsAndTRansferableCoins.coins) + Number(checkIfHaveCoinsAndTRansferableCoins.coins_transferable) >= coinsToPay) {
      ...
      if (Number(checkIfHaveCoinsAndTRansferableCoins.coins) >= remainingCoins) {
        ...
        await accounts.query().update({
          coins: (Number(checkIfHaveCoinsAndTRansferableCoins.coins) - remainingCoins),
        ...
        await accounts.query().update({ coins: 0 }).where({ id: data.account_id });
