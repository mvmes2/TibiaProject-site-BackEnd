...
module.exports = app => {
	...
	const PagseguroCreatePaymnentController = async (req, res) => {
		...
		try {
			...
			dataFront.value = dataFront?.cupom_id && cupom?.data?.cupom_type == 2 ? calculateDiscount(productCheck?.data?.unity_value, cupom?.data?.discount_percent_limit) : productCheck?.data?.unity_value;
