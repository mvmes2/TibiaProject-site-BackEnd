...
module.exports = app => {
	...
	const twitch = async (req, res) => {
		...
		const removeSensitiveInfos = livesStremandoTibiaProject.map((item) => {
			if (item.id && item.user_login && item.user_id) {
				delete item.id;
				delete item.user_id;
				delete item.user_login;
				return item;
			}
			return item;
		});
		cacheTwitchLastUpdated = moment();
		cacheTwitchData = removeSensitiveInfos;

		return res.status(200).send(removeSensitiveInfos);
	}
	...
	const getOfficialStreamersChannelInfo = async (req, res) => {
		try {
			...
			const filteredOfficialListNoSensitiveInfos = filteredOfficialList.map((item) => {
			...
			cacheGetOfficialStreamersChannelInfoData = filteredOfficialListNoSensitiveInfos;

			return res.status(200).send(filteredOfficialListNoSensitiveInfos);
		} catch (err) {
			console.log(err);
