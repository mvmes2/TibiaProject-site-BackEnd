...
module.exports = app => {
  ...
  const StripesCreateCheckoutController = async (req, res) => {
    ...
    try {
      ...
      data.unity_value = data?.cupom_id && cupom?.data?.cupom_type == 2 ? calculateDiscount(productCheck?.data?.unity_value, cupom?.data?.discount_percent_limit) : productCheck?.data?.unity_value;
  ...
  const StrpesInsertCoinsToApprovedPayment = async (req, res) => {
    ...
    const data = req.body;
    const getPayer = await Payer.GetPayerAtList(data.id);
    console.log('como ta vindo o payer em aprovved to insert? ', getPayer);
    console.log('o que vem no insertapproved? ', data);
    const resp = await insertCoinsAtAccountToApprovedPayment(data, null, getPayer?.payerData?.cupom_id);
    res.status(resp.status).send({ message: resp.message });
  }
