...
module.exports = app => {
  ...
  const CreatePixPayment = async (data) => {
    ...
    data.value = data?.cupom_id && cupom?.data?.cupom_type == 2 ? Number(calculateDiscount(productCheck?.data?.unity_value, cupom?.data?.discount_percent_limit).toFixed(2)) : productCheck?.data?.unity_value;
    console.log('valor final = ', data.value);
    try {
      const body = {
      ...
      if (data?.action === 'payment.updated' && paymentId) {
        ...
        await api.get(`/payments/${paymentId}`, { headers }).then(async (resp) => {
          ...
          if (resp.data.status === 'approved') {
            ...
            setTimeout(async () => {
              await insertCoinsAtAccountToApprovedPayment(approvedID, null, GLOBAL_USER_DATA_TO_PAYMENT?.cupom_id);
            }, 1000);
          }
