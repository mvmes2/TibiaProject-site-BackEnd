...
const insertCoinsAtAccountToApprovedPayment = async (paymentID, pagseguroEmail, cupom_id) => {
  ...
  try {
    ...
    /////////////////////////////////////////////////// Founder's Pack.
      ...
      try {
        const getPreviousPackNumberAmmountToSumm = await accounts.query().select('silver_pack', 'gold_pack', 'diamond_pack', 'coins', 'coins_transferable').where({ id: accToPay.account_id }).first();
      ...
      const getPreviousAmmountToSumm = await accounts.query().select('coins', 'coins_transferable').where({ id: accToPay.account_id }).first();
